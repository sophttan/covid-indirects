...
housing <- housing %>% filter(Night>="2020-03-01")
gc()
...
housing_relevant <- housing_relevant %>% ungroup() %>% mutate(Day=Night+1)
gc()

inf_housing_full <- inf_eligible %>% full_join(housing_relevant, by=c("ResidentId")) %>% filter(Day.x-Day.y<=9 & Day.x-Day.y>=6) %>% 
...
write_csv(cases_final, "D:/CCHCS_premium/st/indirects/cases6-9daysame-roommate-040824.csv")
...
for(i in seq(1:35)) {
  ...
  test_housing_full <- test_housing_full %>% filter(Day.x-Day.y<=9 & Day.x-Day.y>=6) %>% 
  ...
  total_excluded_movementroommate <- total_excluded_movementroommate + (test_2 %>%filter(!all(Roommate==first(Roommate)))%>%group_keys()%>%nrow())
...
write_csv(test_final, "D:/CCHCS_premium/st/indirects/control6-9daysame-roommate-040824.csv")
