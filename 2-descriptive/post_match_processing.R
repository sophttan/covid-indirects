...
d <- read_csv("matching_data_071223/matching_data_allvacc_infvacc071223_part1.csv") %>% 
  rbind(read_csv("matching_data_071223/matching_data_allvacc_infvacc071223_part2.csv"))
d <- d %>% mutate(id=1:n()) %>%
...
all <- all %>% mutate(final_start = group_start+5, final_end = end + 5)
all %>% names()
...
all_survival <- all_survival %>% 
  select(id, subclass, treatment, primary, secondary, Institution, BuildingId, InstBuild, inf.primary, inf.secondary, 
         vacc.primary, vacc.secondary, final_start, final_end, survival_time, status) %>%
...
write_csv(all_survival_inf_clean_vacc_demo_risk, "survival_data/allvacc_infvacc071223.csv")
