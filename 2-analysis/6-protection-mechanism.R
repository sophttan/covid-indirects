...
check_test_roommate_summary <- check_test_roommate_filtered %>% filter((!has_test&Day%>%is.na())|(!has_test&(Day==min(Day)))|(test.Day-Day <= num_days & test.Day-Day>0))
check_test_roommate_summary <- check_test_roommate_summary %>% mutate(Result=if_else(!has_test, NA, Result))
...
check_test_roommate_summary %>% mutate(has_either = if_else(has.vacc.roommate.binary>0|has.prior.inf.roommate==1, 1, 0)) %>%
  group_by(id) %>% 
  summarise_all(first) %>% select(!Day) %>%
