...
inc <- read_csv("incidence_final_pos2days.csv")
inf <- read_csv("final_sample100722_nopcr_pos2days.csv") 
d <- prep_data(inc, inf)
...
matched %>% write_csv("matched_data_alternative_postest2days.csv")


# infectious period starts 2 days before first postive test, lasts 5 days
inc <- read_csv("incidence_final_nonegtest.csv")
inf <- read_csv("final_sample100722_nopcr_nonegtest.csv") 
d <- prep_data(inc, inf)
gc()
...
matched %>% group_by(subclass) %>% arrange(subclass, desc(treatment)) %>% 
  ...
  filter(treatment==1|(abs(first(Day)-Day)>day_dist|abs(first(ps)-ps)>ps_dist)) %>% filter(n()>1)
matched <- reweigh_matches(matched, ps_dist, day_dist)
matched %>% write_csv("matched_data_alternative_nonegtest.csv")
