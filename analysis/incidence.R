...
d <- read_csv("march-data/housing_inf_data_adjusted_roomtype.csv")
...
summary <- most_recent_vacc_status %>% summarise(total_res=n(), any_vacc=sum(num_dose>=1,na.rm=T), 
                                                 fully_vacc = sum(num_dose>=full_vacc & full_vacc>=1),
                                                 boosted = sum(num_dose>full_vacc & full_vacc>=1),
...
library(patchwork)
pdf("incidence_by_facility_subset.pdf")
