...
testing_check <- testing %>%
  group_by(ResidentId) %>% 
  filter(Day >= "2021-12-15") %>% 
  left_join(duration%>%select(ResidentId, last)%>%mutate(last=last+1)) %>%
  mutate(time=as.numeric(difftime(last, "2021-12-15"))+1) %>%
  summarise(tests=n(), rate_testing=tests/first(time)*30)

testing_requirement <- testing_check%>%filter(rate_testing>=1)
...
avg_1_test <- over14_noinf %>% 
  mutate(test1=ResidentId.1 %in% testing_requirement$ResidentId,
         test2=ResidentId.2 %in% testing_requirement$ResidentId) %>%
  filter(test1|test2)

avg_1_test <- avg_1_test %>% 
  left_join(infections, by=c("ResidentId.1"="ResidentId", "num_pos_adjusted.1"="num_pos")) %>% 
  rename("infDay.1"="Day") %>%
  left_join(infections, by=c("ResidentId.2"="ResidentId", "num_pos_adjusted.2"="num_pos")) %>% 
  rename("infDay.2"="Day") 

avg_1_test <- avg_1_test %>% 
  left_join(vacc, by=c("ResidentId.1"="ResidentId", "num_dose_adjusted.1"="num_dose")) %>% 
  rename("vaccday.1"="Date") %>% 
  left_join(vacc, by=c("ResidentId.2"="ResidentId", "num_dose_adjusted.2"="num_dose")) %>% 
  rename("vaccday.2"="Date")

avg_1_test <- avg_1_test %>% left_join(demo, by=c("ResidentId.1"="ResidentId"))
avg_1_test <- avg_1_test %>% left_join(demo, by=c("ResidentId.2"="ResidentId"), suffix=c(".1", ".2"))
avg_1_test

avg_1_test <- avg_1_test %>% filter(inf.1 & inf.2)

final <- avg_1_test %>% rowwise() %>% mutate(duration_interval = interval(first, last)) 
final <- final %>% ungroup() %>% mutate(label=1:nrow(.))
...
find_survival <- function(tbl) {
  add_testing <- function(tbl) {
    tbl %>% left_join(testing, by=c("primary"="ResidentId"))
  }
...
survival_data <- function(tbl) {
  ...
  tbl <- tbl %>% 
    mutate(intersection=interval(final_start, final_end),
           time_since_inf.primary=(difftime(final_start, infDay.primary, units="days")%>%as.numeric())/30.417,
           time_since_inf.secondary=(difftime(final_start, infDay.secondary, units="days")%>%as.numeric())/30.417,
           time_since_vacc.primary = ((final_start-vaccday.primary)%>%as.numeric())/30.417,
           time_since_vacc.secondary = ((final_start-vaccday.secondary)%>%as.numeric())/30.417)
  
  tbl <- tbl %>% left_join(risk, by=c("primary"="ResidentId")) %>% 
  ...
  tbl <- tbl %>% left_join(risk, by=c("secondary"="ResidentId")) %>% 
    mutate(overlap_risk = intersect(intersection, risk_interval)) %>%
    filter(!is.na(overlap_risk)) %>% 
    mutate(days_risk=time_length(overlap_risk, unit = "day")+1) %>% 
    group_by(id) %>% 
...
for (i in 11:50) {
  ...
  for_matching <- avg_1_test %>% 
    mutate(secondary = if_else(primary==ResidentId.1, ResidentId.2, ResidentId.1),
           vacc.primary=if_else(primary==ResidentId.1, num_dose_grouped.1, num_dose_grouped.2),
           vacc.secondary=if_else(primary==ResidentId.1, num_dose_grouped.2, num_dose_grouped.1),
           vacc.primary.doses=if_else(primary==ResidentId.1, num_dose_adjusted.1, num_dose_adjusted.2),
           vacc.secondary.doses=if_else(primary==ResidentId.1, num_dose_adjusted.2, num_dose_adjusted.1),
           vaccday.primary=if_else(primary==ResidentId.1, vaccday.1, vaccday.2),
           vaccday.secondary=if_else(primary==ResidentId.1, vaccday.2, vaccday.1),
           infDay.primary=if_else(primary==ResidentId.1, infDay.1, infDay.2),
           infDay.secondary=if_else(primary==ResidentId.1, infDay.2, infDay.1),
           age.primary=if_else(primary==ResidentId.1, age.1, age.2),
           age.secondary=if_else(primary==ResidentId.1, age.2, age.1)) 
  
  for_matching <- for_matching %>% ungroup() %>%
    mutate(treatment = ifelse(vacc.secondary==0, 1, 0)) %>% 
    mutate(duration_interval = interval(first, last)) 

  matched <- matching(for_matching) 
