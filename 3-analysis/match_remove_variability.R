...
test <- d_subset %>% 
  mutate(possible_control = any(test&vacc==0)) %>% 
  summarise_all(first) %>%
...
remove <- test %>% filter(!possible_control) %>% 
  ...
  mutate(overlap=intersect(duration_interval.x, duration_interval.y)%>%time_length("day")) %>% 
  group_by(id.x) %>% filter(all(overlap%>%is.na())) %>% group_keys()
...
resident_info <- d_subset %>% select(!names(unit_info)) %>% arrange(id, desc(test), num_dose_grouped) 
...
filter_controls <- function(tbl) {
  tbl <- tbl %>% mutate(label=1:n())
  tblselect <- tbl %>% select(label, treatment, duration_interval)
  treatment <- tblselect %>% filter(treatment==1) 
  control <- tblselect %>% filter(treatment==0) 
  
  no_intersection <- treatment %>% cross_join(control) %>% 
    mutate(intersection=time_length(intersect(duration_interval.x, duration_interval.y), "day")%>%as.numeric()+1) %>% 
    group_by(label.x) %>% 
    filter(all(intersection<7,na.rm=T))
  
  tbl %>% filter(!label %in% no_intersection$label.x)
}
...
all <- resident_info %>% mutate(both_test_diff_vacc=all(test)&num_dose_grouped[1]!=num_dose_grouped[2]) %>% 
  arrange(id, desc(test), vacc) %>% 
...
assignment <- c(rbind(res[optimum,], abs(res[optimum,]-1)))
all_mixed <- all_mixed %>% ungroup() %>% mutate(type=factor(assignment, labels =c("primary","secondary")))
