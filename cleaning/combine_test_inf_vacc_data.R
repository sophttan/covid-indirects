...
d <- test_inf %>% 
  full_join(vacc %>% select(!c(Result, Month)), by=c("ResidentId", "Day" = "Date")) %>% 
...
vacc[!(vacc$ResidentId %in% (test_inf$ResidentId %>% unique())),]
...
total_inf_time <- unique_vacc_infections %>% group_by(Month) %>% summarise(Day=mean(Day), inf=n())
total_inf_time %>% ggplot(aes(Day, inf)) + geom_line() + ylab("Confirmed cases (monthly)") + xlab("Month")
