...
# mechanism 
relevant<- matched_infvacc_roommate %>% 
...
check_test <- relevant %>% left_join(test_data, by=c("ResidentId"="ResidentId")) %>% group_by(id)

infection <- read_csv("D:/CCHCS_premium/st/cleaned-data/infection_data022624.csv")

check_test %>% 
  filter(all(Day>test.Day|test.Day-Day>=5)) %>% summarise_all(first) %>% group_by(case, has.vacc.roommate.binary) %>% 
  summarise(n=n())%>%ungroup()%>%mutate(prop=n/c(294, 4310, 382, 4222))

check_test <- check_test %>% mutate(has_test=any(test.Day-Day <= 4 & test.Day-Day>0,na.rm=T))
check_test_summary <- check_test %>% filter((!has_test&Day%>%is.na())|(!has_test&(Day==min(Day)))|(test.Day-Day <= 4 & test.Day-Day>0))
...
relevant %>% left_join(infection %>% select(ResidentId, CollectionDate), by=c("Roommate"="ResidentId")) %>% 
  filter(test.Day-CollectionDate<=4 & test.Day-CollectionDate>0) %>% 
  mutate(has_either = if_else(has.vacc.roommate.binary==1|has.prior.inf.roommate==1, 1, 0)) %>%
  group_by(case, has_either) %>% summarise(n=n()) %>%
  ungroup() %>%
...
relevant %>% left_join(infection %>% select(ResidentId, CollectionDate), by=c("Roommate"="ResidentId")) %>% 
  filter(test.Day-CollectionDate<=14 & test.Day-CollectionDate>0) %>%
  mutate(time=(test.Day-CollectionDate)%>%as.numeric()) %>% 
  select(ResidentId, Roommate, test.Day, CollectionDate, time) %>% 
  filter(time>9)
