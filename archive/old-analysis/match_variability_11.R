...
generate_distance_matrix <- function(tbl) {
  ...
  distance_propensity[!duration_overlap_wide|!eligible_wide] <- 1000
  distance_propensity
}
...
generate_assignment <- function(i, assignments, data_mixed, data_other) {
  ...
  filtered <- filtered %>% mutate(age.primary = cut(age.primary, breaks=c(0,50,Inf), right=F),
                                  risk.primary = cut(risk.primary, breaks=c(0, 2,Inf), right=F),
                                  time_since_inf.primary = cut(time_since_inf.primary, breaks=c(0, 12,Inf), right=F),
                                  time_since_vacc.primary = cut(time_since_vacc.primary, breaks=c(0, 12,Inf), right=F))
  filtered
}
...
for (building in buildings) {
  ...
  test <- d_building %>% 
    ...
    summarise_all(first) %>%
    mutate(duration_interval=interval(first, last)) %>%
    select(id, first, duration_interval, possible_control) 
  ...
  remove <- test %>% filter(!possible_control) %>% 
    ...
    group_by(id.x) %>% filter(all(overlap<1, na.rm=T) & all(eligible>=90)) %>% group_keys()
  ...
  summary_data4 <- final_processed4%>%group_by(i)%>%
    summarise(control=unique(subclass)%>%length()) 
  print(head(summary_data4))
  
  optimum <- (summary_data4%>%arrange(desc(control)))$i[1]
  print(optimum)
